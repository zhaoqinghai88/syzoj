...
export default class Contest extends Model {
  ...
  allowedSeeingOthers() {
    if (this.type === 'acm' || this.type === 'crt') return true;
    else return false;
  }

  allowedSeeingScore() { // If not, then the user can only see status
    if (this.type === 'ioi' || this.type === 'crt') return true;
    else return false;
  }

  allowedSeeingResult() { // If not, then the user can only see compile progress
    if (this.type === 'ioi' || this.type === 'acm' || this.type === 'crt') return true;
    else return false;
  }
